...
export class AppointmentService {
	...
	async createAppointment(userId: string, dto: CreateAppointmentDto) {
		...
		if (dto.staffId) {
			...
			if (eligibility.ok) {
				...
				message = eligibility.reason || 'Staff unavailable, added to waiting queue';
			}
		} else {
	...
	async updateAppointment(
		...
		if (status === AppointmentStatus.CANCELLED) {
			...
			return ResponseUtil.success(updated, 'Appointment cancelled successfully');
		}
	...
	async cancelAppointment(userId: string, id: string) {
		...
		return ResponseUtil.success(cancelled, 'Appointment cancelled successfully');
	}
